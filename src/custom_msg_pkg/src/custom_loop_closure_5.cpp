...
// LOOP CLOSURES
      ...
      // Timer 5-1
      timer51_ = this->create_wall_timer(
      50000ms, std::bind(&LoopClosurePublisher::timer51_callback, this));

      // Timer 5-2
      timer52_ = this->create_wall_timer(
      50000ms, std::bind(&LoopClosurePublisher::timer52_callback, this));

      // Timer 5-3
      timer53_ = this->create_wall_timer(
      50000ms, std::bind(&LoopClosurePublisher::timer53_callback, this));

      // Timer 5-4
      timer54_ = this->create_wall_timer(
      50000ms, std::bind(&LoopClosurePublisher::timer54_callback, this));

      // Timer 5-6
      timer56_ = this->create_wall_timer(
      50000ms, std::bind(&LoopClosurePublisher::timer56_callback, this));

      // Timer 5-7
      timer57_ = this->create_wall_timer(
      50000ms, std::bind(&LoopClosurePublisher::timer57_callback, this));

      // Timer 5-8
      timer58_ = this->create_wall_timer(
      50000ms, std::bind(&LoopClosurePublisher::timer58_callback, this));
    }
...
// LOOP CLOSURES FUNCTIONS
  ...
  private:
    void timer51_callback() const
    {
      auto message = cslam_common_interfaces::msg::InterRobotLoopClosure();
      float dx = 0.0;
      float dy = 0.0;
      
      message.robot0_keyframe_id = odom_5.id;
      message.robot0_id = 4;
      message.robot1_keyframe_id = odom_1.id;
      message.robot1_id = 0;
      
      dx = odom_1.odom.pose.pose.position.x - odom_5.odom.pose.pose.position.x;
      dy = odom_1.odom.pose.pose.position.y - odom_5.odom.pose.pose.position.y;
  ...
  private:
    void timer52_callback() const
    {
      ...
      dx = odom_2.odom.pose.pose.position.x - odom_5.odom.pose.pose.position.x;
      dy = odom_2.odom.pose.pose.position.y - odom_5.odom.pose.pose.position.y;
    ...
    void timer53_callback() const
    {
      ...
      dx = odom_3.odom.pose.pose.position.x - odom_5.odom.pose.pose.position.x;
      dy = odom_3.odom.pose.pose.position.y - odom_5.odom.pose.pose.position.y;
    ...
    void timer54_callback() const
    {
      ...
      dx = odom_4.odom.pose.pose.position.x - odom_5.odom.pose.pose.position.x;
      dy = odom_4.odom.pose.pose.position.y - odom_5.odom.pose.pose.position.y;
    ...
    void timer56_callback() const
    {
      ...
      dx = odom_6.odom.pose.pose.position.x - odom_5.odom.pose.pose.position.x;
      dy = odom_6.odom.pose.pose.position.y - odom_5.odom.pose.pose.position.y;
    ...
    void timer57_callback() const
    {
      ...
      dx = odom_7.odom.pose.pose.position.x - odom_5.odom.pose.pose.position.x;
      dy = odom_7.odom.pose.pose.position.y - odom_5.odom.pose.pose.position.y;
    ...
    void timer58_callback() const
    {
      ...
      dx = odom_8.odom.pose.pose.position.x - odom_5.odom.pose.pose.position.x;
      dy = odom_8.odom.pose.pose.position.y - odom_5.odom.pose.pose.position.y;
    ...
    rclcpp::TimerBase::SharedPtr timer51_;
    rclcpp::TimerBase::SharedPtr timer52_;
    rclcpp::TimerBase::SharedPtr timer53_;
    rclcpp::TimerBase::SharedPtr timer54_;
    rclcpp::TimerBase::SharedPtr timer56_;
    rclcpp::TimerBase::SharedPtr timer57_;
    rclcpp::TimerBase::SharedPtr timer58_;
