...
class PubSub : public rclcpp::Node
{
  public:
    ...
    : Node("pubsub")
    {
      subscription_ = this->create_subscription<nav_msgs::msg::Odometry>("/bot6/odom", 100,
      std::bind(&PubSub::topic_callback, this, _1));
      subscription_noisy_ = this->create_subscription<nav_msgs::msg::Odometry>("/bot6/noisy_odom", 100,
      std::bind(&PubSub::topic_callback_noisy, this, _1));

      publisher_ = this->create_publisher<cslam_common_interfaces::msg::KeyframeOdom>("/r5/cslam/keyframe_odom_ideal", 1000);
      publisher_noisy_ = this->create_publisher<cslam_common_interfaces::msg::KeyframeOdom>("/r5/cslam/keyframe_odom", 1000);
    }
